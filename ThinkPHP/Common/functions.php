...
function array_key( $array, $key )
{
    ...
    {
        if(strpos($key,'-')>-1){
            list($nkey,$nvalue) = explode('-',$key);
            $new_array[ $item[$nkey] ] = $item[$nvalue];
        }else{
            $new_array[$key] = $item;
        }
    }
...
//取得token
function token_get( $name='hash' ){
    session_start();
    $key = guid();
    $value = guid();
    $_SESSION[$name][$key] = $value;
    return '<input id="'.$name.'" type="hidden" name="'.$name.'" value="'. "{$key}_{$value}" .'" />';
}
//检查token
function token_check( $name='hash' )
{
    session_start();
    if( !isset($_POST[$name]) || !isset($_SESSION[$name]) ){//页面中没有传token的隐藏域
        return false;
    }

    list($key,$value) = explode('_',$_POST[$name]);
    if( isset($_SESSION[$name][$key]) && $value && $_SESSION[$name][$key] === $value ){//值相同认证通过
...
//微信认证跳转连接
function wechat_link( $url )
{
    $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . C('APPID') . '&redirect_uri=' . urlencode('http://wx.kmg898.com'. '/'. $url);
    //$url = 'http://wx.kmg898.com'. '/'. $url;
    redirect( $url );
}
 ...
 * @return bool
 */
function integral_change( $user_id, $key, $type="+" )
{
    $jf_set = include GMLPATH."/write_configs/jf_set.php";
    ...
    if( $type=="+" )//获取积分
    {
        $last_info = M('integral')->where("uid=$user_id")->order("dtime DESC")->find();
        if(!empty($last_info)){
            $remaining = $last_info["remaining"]+$jf_set[$key]["point"];
        }else{
            $remaining = $jf_set[$key]["point"];
        }
        $result = M("integral")->add(array(
            "uid"=>$user_id, "type"=>$jf_set[$key]["type"], "change"=>$jf_set[$key]["point"],
            "remaining"=>$remaining,"remark"=>$jf_set[$key]["remark"],"dtime"=>time(),"type_key"=>$key
            ));
